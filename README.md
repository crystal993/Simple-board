...
해결 방법 : vervel setting에서 환경변수로 env key값과 value값을 넣어줘야 했습니다.
  
- 컴포넌트 설계 문제로 인한 기능 구현의 어려움 
컴포넌트 구조를 설계할 때, Detail 한 페이지에서 등록, 수정, detail페이지로 분기해서 이동하는 3가지 기능을 구현하는 것으로 설계했습니다. 
그 결과 Detail페이지 컴포넌트 내의 코드가 증가하였고, 각각의 기능을 구현하기 위해 하나의 form이 등록, 읽기, 수정 기능을 수행하기 위해 useMatch hook으로 등록, 수정, 읽기 3가지 기능을 수행하기 위해 조건부 렌더링을 해야하는 어려움이 발생했습니다. 
